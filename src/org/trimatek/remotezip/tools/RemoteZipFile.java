...
public class RemoteZipFile {
    ...
    public boolean load(String path) throws IOException {
        ...
        req.setRequestProperty("Range", "bytes=" + centralOffset + "-" + centralOffset + centralSize);
        req.connect();
    ...
    private long readRemoteFileLength(URL url) {
    ...
    public static int readAll(byte[] bb, int p, int sst, InputStream s) throws IOException {
        int ss = 0;
        while (ss < sst) {
    ...
    public InputStream getInputStream(RemoteZipEntry entry) throws MalformedURLException, IOException {
        ...
        if (index < 0 || index >= entries.length || entries[index].getName() != entry.getName()) {
            throw new IndexOutOfBoundsException();
        }

        HttpURLConnection req = (HttpURLConnection) new URL(baseUrl).openConnection();
        int limit = (int) (entry.getOffset() + entry.getCompressedSize() + 16 + 65536 * 2);
        if (limit >= maxFileOffset) {
            limit = maxFileOffset - 1;
        }

        req.setRequestProperty("Range", "bytes=" + entry.getOffset() + "-" + limit);
        InputStream baseStream = req.getInputStream();

        skipLocalHeader(baseStream, entries[index]);

        InputStream istr = new PartialInputStream(baseStream, req, entries[index].getCompressedSize());
    ...
    private void skipLocalHeader(InputStream baseStream, RemoteZipEntry entry) throws IOException {
